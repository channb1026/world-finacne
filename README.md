...
当前资讯区已经不是单纯的新闻列表，而是带有 monitor 特征的事件流：

- 会展示“今日主线摘要”，把当前主线、主题集中度、区域偏向、最近活跃主题收拢成一句话
- 会展示当前市场主线，例如美元/汇率、原油/通胀、风险偏好、A 股盘面
- 每条事件会展示与当前主线的关联原因
- 每条事件会展示更可能影响的资产维度，例如汇率、利率、股市、大宗、中国宏观、A 股
- 每条事件会标记自身阶段，例如“新出现”“持续发酵”“持续关注”
- 会显示最近 `1H / 4H / 24H` 的主线回看
- 会显示当前一屏资讯的主题集中和区域偏向
- 会支持事件展开，查看同一事件下的多来源报道
...
- 防重入与防请求堆积
- 页面不可见时暂停轮询
- 公共类型已抽到 `frontend/src/types/data.ts`
- 时间格式化等重复逻辑已抽成公共工具，避免多面板拷贝
...
- 多源 fallback
- 刷新超时保护
- Yahoo Finance 快照退避 / 熔断
- 新闻 RSS 抓取并发限制
- `/api/dashboard` 部分成功可返回
- 新闻去重、聚类、分类
- 新闻事件化呈现与主线摘要
- CORS allowlist
- 基础限流
